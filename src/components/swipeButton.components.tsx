...
const SwipeButton: React.FC<SwipeButtonProps> = ({ guests, onConfirm, onCancel, shouldContinueRight, id }) => {
  ...
  useEffect(() => {
    // Фильтрация пустых элементов при инициализации
    const filteredGuests = guests.filter(guest => guest.full_name.trim() !== '');
    setCurrentGuests(filteredGuests);
  ...
  const handleConfirm = () => {
    // Фильтрация пустых элементов перед отправкой
    const filteredGuests = currentGuests.filter(guest => guest.full_name.trim() !== '');
